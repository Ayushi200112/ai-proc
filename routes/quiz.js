...
/*router.post('/create', authenticateToken, isCreator, async (req, res) => {
...
const isCreator = (req, res, next) => {
  console.log('User in isCreator middleware:', req.user); // Debugging
  if (!req.user) {
...
router.get('/creator', authenticateToken, isCreator, async (req, res) => {
  try {
    console.log('Fetching quizzes for creator:', req.user.userId);
    const quizzes = await Quiz.find({ creator: req.user.userId });
    res.status(200).json(quizzes);
  } catch (error) {
...
router.post('/create', authenticateToken, isCreator, async (req, res) => {
  try {
    ...
    console.log('Creating quiz for:', req.user.userId); // Debugging
...
// ✅ **Fix: Apply `authenticateToken` before `isCreator` for editing and deleting quizzes**
router.put('/edit/:quizId', authenticateToken, isCreator, async (req, res) => {
  const { title, questions, duration } = req.body;
  try {
    const quiz = await Quiz.findByIdAndUpdate(
      req.params.quizId,
...
router.delete('/delete/:quizId', authenticateToken, isCreator, async (req, res) => {
  try {
    await Quiz.findByIdAndDelete(req.params.quizId);
    res.status(200).json({ message: 'Quiz deleted successfully' });
  } catch (error) {
...
router.post('/attempt/:quizId', authenticateToken, async (req, res) => {
  try {
